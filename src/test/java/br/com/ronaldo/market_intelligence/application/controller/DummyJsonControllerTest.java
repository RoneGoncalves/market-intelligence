...
class DummyJsonControllerTest {
    ...
    @MockBean
    private BestSellingProductService productService;
    ...
    void shouldCreateUserSuccessfully() throws Exception {
        ...
        verify(createUserService, times(1)).execute(any());
    }
    ...
    void shouldReturn409WhenUserAlreadyExists() throws Exception {
        ...
        when(createUserService.execute(any()))
    ...
    void shouldReturnTicketMedioSuccessfully() throws Exception {
        TicketMedioResponseDto response = new TicketMedioResponseDto(
                50, "R$ 20.085,36", "R$ 18.141,28", "R$ 160.289,86",
                "R$ 36,28", 12, "R$ 37.754,73", "R$ 33.686,79",
                "R$ 160.289,86", "R$ 36,28",
                "Insight 1", "Insight 2"
        );

        when(ticketMedioService.execute()).thenReturn(response);
        ...
        verify(ticketMedioService, times(1)).execute();
    }

    @Test
    void shouldReturn502WhenTicketMedioExternalApiFails() throws Exception {
        when(ticketMedioService.execute())
                .thenThrow(new ExternalApiException("dummy fail", null));
    ...
    void shouldDeleteUserSuccessfully() throws Exception {
        ...
        doNothing().when(deleteUserService).excluirUsuario(id);

        mockMvc.perform(delete("/api/delete_user/{id}", id))
                .andExpect(status().isNoContent());

        verify(deleteUserService, times(1)).excluirUsuario(id);
    }
